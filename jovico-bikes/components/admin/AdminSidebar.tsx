...
import {
  LayoutDashboard, ShoppingBag, FileText, Settings, LogOut,
  Zap, Wrench, ShoppingCart, Mail, Menu, X, Users, Tag,
  BookOpen, MessageSquare, BarChart3, Bell,
} from "lucide-react";
...
export function AdminSidebar() {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-4 py-5 border-b border-slate-100">
        ...
        {!collapsed && (
          <div>
            ...
            <div className="text-[10px] text-slate-400 font-medium uppercase tracking-wider mt-0.5">Admin</div>
          </div>
        )}
      </div>
      ...
      <div className="border-t border-slate-100 p-3 space-y-1">
        <Link href="/" target="_blank" className={cn("admin-nav-item", collapsed ? "justify-center px-2" : "")}>
        ...
        <button
          ...
          className={cn("admin-nav-item w-full text-left text-red-500 hover:bg-red-50", collapsed ? "justify-center px-2" : "")}
        >
  ...
  return (
    <>
      ...
      {/* Mobile drawer */}
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-30 flex pt-14" onClick={() => setMobileOpen(false)}>
          <div className="w-64 bg-white h-full border-r border-slate-100 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <SidebarContent />
          </div>
