...
const CATEGORIES = [
  "CITY_BIKE", "MOUNTAIN_BIKE", "CARGO_BIKE",
  "FOLDING_BIKE", "ROAD_BIKE", "ACCESSORY", "PART", "APPAREL",
];

const SPEC_KEYS = [
  "motor", "battery", "range", "topSpeed", "chargeTime",
  "weight", "frame", "brakes", "gears", "display", "suspension",
];

interface Product {
  id: string; name: string; slug: string; description: string;
  price: any; salePrice: any; sku: string; stock: number;
  category: string; type: string; brand: string | null;
  specs: any; featured: boolean; published: boolean;
}
...
export function ProductEditor({ product, mode }: Props) {
  ...
  function removeSpec(key: string) {
    setSpecs((s) => { const n = { ...s }; delete n[key]; return n; });
  }
  function addSpec() {
    ...
    <div className="grid lg:grid-cols-[1fr_260px] gap-6">
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-6 space-y-4">
          ...
          <div>
            ...
            <input type="text" value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))}
              className="jv-input" placeholder="Jovico City Cruiser Pro" />
          </div>
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Description *</label>
            <textarea value={form.description} onChange={(e) => setForm(f => ({ ...f, description: e.target.value }))}
              rows={4} className="jv-input resize-none" placeholder="Describe the product..." />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.sku} onChange={(e) => setForm(f => ({ ...f, sku: e.target.value }))}
                className="jv-input font-mono" placeholder="JVC-CC-PRO-001" />
            </div>
            <div>
              ...
              <input type="text" value={form.brand} onChange={(e) => setForm(f => ({ ...f, brand: e.target.value }))}
                className="jv-input" placeholder="Jovico" />
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input type="number" value={form.price} onChange={(e) => setForm(f => ({ ...f, price: e.target.value }))}
                className="jv-input" placeholder="485000" min={0} />
            </div>
            <div>
              ...
              <input type="number" value={form.salePrice} onChange={(e) => setForm(f => ({ ...f, salePrice: e.target.value }))}
                className="jv-input" placeholder="Optional" min={0} />
            </div>
            <div>
          ...
          <div className="grid grid-cols-1 gap-3">
            {SPEC_KEYS.map((key) => (
              <div key={key} className="flex items-center gap-3">
                ...
                <input type="text" value={specs[key] ?? ""} onChange={(e) => updateSpec(key, e.target.value)}
                  className="jv-input flex-1 text-sm" placeholder={`e.g. ${key === "motor" ? "500W Hub Motor" : "..."}`} />
              </div>
            ))}
            {Object.keys(specs).filter(k => !SPEC_KEYS.includes(k)).map((key) => (
              <div key={key} className="flex items-center gap-3">
                <label className="w-32 text-xs font-semibold text-slate-600 capitalize shrink-0">{key}</label>
                <input type="text" value={specs[key]} onChange={(e) => updateSpec(key, e.target.value)}
                  className="jv-input flex-1 text-sm" />
                <button type="button" onClick={() => removeSpec(key)}
                  className="p-2 text-red-400 hover:text-red-600"><Minus className="w-3.5 h-3.5" /></button>
              </div>
            ))}
          </div>
        </div>
        ...
        <button type="button" onClick={handleSave} disabled={saving}
          ...
          {saving ? <><Loader2 className="w-5 h-5 animate-spin" /> Saving...</> : <><Save className="w-5 h-5" /> {mode === "create" ? "Create Product" : "Save Changes"}</>}
        </button>
      </div>
