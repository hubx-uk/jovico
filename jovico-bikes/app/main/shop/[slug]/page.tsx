...
import {
  ArrowLeft, CheckCircle2, Zap, Shield, Clock,
  MessageCircle, Phone, ChevronRight, Star
} from "lucide-react";
    ...
    <>
      ...
      <div className="pt-24 pb-4 bg-white border-b border-slate-100">
        <div className="jv-container">
          <nav className="flex items-center gap-2 text-sm text-slate-500">
            ...
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-slate-900 font-medium truncate max-w-48">{product.name}</span>
          </nav>
        </div>
      </div>
      ...
      <section className="jv-section bg-white">
        <div className="jv-container">
          <div className="grid lg:grid-cols-2 gap-12 xl:gap-16">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-8">
                <AddToCartButton product={{ id: product.id, name: product.name, price: Number(product.price), slug: product.slug }} />
              ...
              <div className="grid grid-cols-3 gap-3 mb-8">
                {[
                  ...
                  <div key={t.label} className="flex flex-col items-center gap-1.5 p-4 rounded-2xl bg-slate-50 text-center">
          ...
          {specs && Object.keys(specs).length > 0 && (
            <div className="mt-16">
              <h2 className="text-2xl font-extrabold text-slate-900 mb-6">Technical Specifications</h2>
      ...
      {related.length > 0 && (
        <section className="jv-section bg-slate-50">
          <div className="jv-container">
            <h2 className="text-2xl font-extrabold text-slate-900 mb-8">You Might Also Like</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {related.map((p) => (
                <Link key={p.id} href={`/shop/${p.slug}`} className="group jv-card bg-white overflow-hidden">
                  ...
                  <div className="p-5">
                    <p className="text-xs text-slate-400 mb-1">{p.category.replace(/_/g, " ")}</p>
                    <h3 className="font-bold text-slate-900 group-hover:text-green-600 transition-colors mb-2">{p.name}</h3>
                    <span className="font-extrabold text-slate-900">{formatNaira(Number(p.price))}</span>
                  </div>
                </Link>
              ))}
            </div>
