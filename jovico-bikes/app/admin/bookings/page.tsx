        ...
        {["PENDING", "CONFIRMED", "COMPLETED", "CANCELLED"].map((status) => {
          ...
          return (
            <div key={status} className="bg-white rounded-2xl border border-slate-100 p-5">
              <div className={`jv-badge text-xs font-semibold mb-2 ${statusColors[status]}`}>
                {status}
              </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  ...
                  <th className="text-left px-4 py-3.5 font-semibold text-slate-600">Service</th>
                  <th className="text-left px-4 py-3.5 font-semibold text-slate-600">Date</th>
                  <th className="text-left px-4 py-3.5 font-semibold text-slate-600">Notes</th>
                ...
                {bookings.map((booking) => (
                  <tr key={booking.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-4 text-slate-500 max-w-xs">
                      <p className="text-xs line-clamp-2">{booking.notes || "—"}</p>
                    </td>
                    <td className="px-4 py-4">
                      <AdminBookingStatus id={booking.id} status={booking.status} />
                    </td>
                    <td className="px-4 py-4">
