      ...
      <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
        {posts.length === 0 ? (
          <div className="py-20 text-center text-slate-400">
            ...
            <p className="text-sm mt-1 mb-6">Create your first blog post to get started.</p>
            <Link href="/admin/blog/new" className="jv-btn-primary">
              <Plus className="w-4 h-4" /> New Post
            </Link>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  <th className="text-left px-6 py-3.5 font-semibold text-slate-600">Title</th>
                  <th className="text-left px-4 py-3.5 font-semibold text-slate-600">Category</th>
                  <th className="text-left px-4 py-3.5 font-semibold text-slate-600">Author</th>
                  <th className="text-left px-4 py-3.5 font-semibold text-slate-600">Status</th>
                  <th className="text-left px-4 py-3.5 font-semibold text-slate-600">Views</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {posts.map((post) => (
                  <tr key={post.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-4">
                      <AdminTogglePost id={post.id} published={post.published} />
                    </td>
                    <td className="px-4 py-4 text-slate-600">{post.views.toLocaleString()}</td>
