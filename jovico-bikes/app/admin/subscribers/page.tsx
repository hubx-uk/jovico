            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  ...
                  <th className="text-left px-4 py-3.5 font-semibold text-slate-600">Name</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {subscribers.map((sub) => (
                  <tr key={sub.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-4 text-slate-600">{sub.name ?? "—"}</td>
                    <td className="px-4 py-4">
                      <span className={`jv-badge text-xs font-semibold ${
                        sub.active ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"
                      }`}>
